...
class AssetsService extends Service {
  async create(params) {
    ...
    const result = await this.app.mysql.insert('assets', {
      assets_id,
      assets_name,
      assets_desc,
      app_id,
  ...
  async list(params) {
    const { app_id } = params;
    const result = await this.app.mysql.select('assets', {
      where: { app_id },
      columns: ['assets_id', 'assets_name', 'assets_desc', 'app_id', 'drag_platform_path', 'create_date']
    });
    return result
  }
  async briefInfo(params) {
    const { assets_id } = params;
    const result = await this.app.mysql.select('assets', {
      where: { assets_id },
      columns: ['assets_id', 'assets_name', 'assets_desc', 'app_id', 'drag_platform_path', 'create_date', 'schema_id', 'custom_code_id']
    });
    return result
  }
  async update(params) {
    const { assets_id, schema_id } = params;
    console.log('--111-schema_id--', schema_id)
    const result = await this.app.mysql.update('assets', {
      schema_id: schema_id
    }, {
      where: {
        assets_id
